...
static std::deque<size_t> jacobIndices(size_t n)
{
    if (n == 0)
...
std::deque<int> fjSort_2(const std::deque<int> &arr)
{
    if (arr.size() <= 1)
        return arr;
    ...
    std::deque<int> mainChain = fjSort_2(aVals);
    ...
    std::deque<std::pair<int, size_t> > partnerPosVec;
    ...
    std::deque<size_t> insertionOrder = jacobIndices(pendElements.size());
    ...
    for (size_t k = 0; k < insertionOrder.size(); ++k)
    {
        ...
        std::deque<int>::iterator pos = std::lower_bound(result.begin(), result.begin() + bound, elementToInsert);
